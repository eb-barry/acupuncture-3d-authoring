...
import { MERIDIANS, POINTS, POINT_BY_CODE, isGbChenglingNaokongPair, isHtShaofuShaochongPair, isKiYinguChangqiangPair, isLiFutuHeliaoPair, isOmittedSurfaceSpan, isRenDuCodePair, meridianById, meridianLineColor, acupointMarkerColor, pointsForMeridian } from './catalog.js'
import {
  BODY_MODELS,
  emptyDocument,
  exportFileName,
...
function maleHtPinkyPair(fromCode, toCode) {
  return isMaleStudioBody() && isHtShaofuShaochongPair(fromCode, toCode)
}

function wantsDigitTipWrap(fromCode, toCode, from, to, normalDot) {
  return maleHtPinkyPair(fromCode, toCode) || isDigitTipWrap(from, to, normalDot)
}
...
function handleSkinHit(event, drag) {
  ...
  if (planePoint) {
    ...
    if (liMale) {
      ...
      const sideOn = Math.abs(look.x) > Math.abs(look.z) * 0.85
      const faceX = Math.abs(from.position[0]) < Math.abs(to.position[0]) ? from.position[0] : to.position[0]
      const notLipJump = (hit) => {
        if (Math.abs(anchor.position[0]) <= Math.abs(faceX) + span * 0.05) return true
        if (
          Math.abs(hit.position[0]) < Math.abs(anchor.position[0]) * 0.55
          && hit.position[2] > Math.max(from.position[2], to.position[2]) * 0.25
        ) {
          return false
        }
        return true
      }
      // Profile: mouse motion is mostly world Z. XY front-cast would ignore it
      // and pin the locator to one neck/cheek hit.
      if (sideOn && alongRay && notLipJump(alongRay)) return alongRay
      if (sideOn) {
        const nearby = closestSkinHit(planeArr, {
          maxDistance: Math.max(statureWorld(0.02), span * 0.16),
          sideX,
          guideNormal: guide,
        })
        if (accept(nearby) && nearby.position[2] >= Math.min(from.position[2], to.position[2]) - span * 0.03) {
          return nearby
        }
      }
...
function isUsableDigitPath(points, from, to) {
  ...
  if (maxPolylineEdge(points) > 0.02) return false
  const chord = dist3(from, to)
  if (chord > 1e-4 && polylineArcLength(arraysFromSkin(points)) > chord * 1.85) return false
  if (!digitPathIsMonotonic(points, from, to, 0.005)) return false
...
function sampleDigitSkinPath(a, b, tipPos, tipNormal, {
  ...
  const accept = (hit, {
    ...
    if (
      minPalmarDot != null
      && new THREE.Vector3(...hit.normal).normalize().dot(palmarVec) < minPalmarDot
    ) return false
  ...
  const palmarCount = Math.min(28, Math.max(12, Math.ceil(span / 0.004) + 8))
  for (let index = 1; index < palmarCount; index += 1) {
    const t = index / palmarCount
    const chord = start.clone().lerp(palmarEnd, t)
  ...
  if (tipOnSkin) {
    accept({ position: toArray(tipPos), normal: tipGuide }, {
      corridorFrom: palmarFrom,
      corridorTo: palmarTo,
      maxOff: 0.012,
      maxStep: 0.012,
...
function snapDigitTipWrap(a, b) {
  ...
  const tipOnSkin = tipHit && isOnDigitSkin(tipHit.position, a.position, b.position, 0.030)
  ...
  const finish = (points) => {
    ...
    const pruned = pruneSharpPolylineTurns(
      pruneBacktracking(arraysFromSkin(points), b.position),
      0.12,
    )
    const vectors = vectorsFromArrays(pruned)
    return isUsableDigitPath(vectors, a.position, b.position) ? vectors : null
  }
  ...
  return finish(marchOnSkinHits(a, b, { hint: distal, stepScale: 0.45 }))
}
...
function snapLiHandleToSkinMale(placed, fromResolved, toResolved, rest = []) {
  ...
  const minZ = Math.min(from[2], to[2]) - span * 0.03
  const legal = (hit) => hit && liHandleOk(hit.position, from, to) && hit.position[2] >= minZ
  const guide = placed.normal || liFutuHeliaoGuide(from, to, 0.5)
  if (liHandleOk(placed.position, from, to)) {
    const near = closestSkinHit(placed.position, {
      maxDistance: Math.max(statureWorld(0.008), span * 0.04),
      sideX: from[0],
      guideNormal: guide,
    })
  ...
  if (legal(hit)) return { position: hit.position, normal: hit.normal }
  return null
}
...
function snapLiFutuHeliaoToSkinMale(a, b, records = [], rest = []) {
  ...
  let usable = ordered
    ...
    .filter(Boolean)
  const drawThrough = (handles) => {
    if (!handles.length) return null
    const spline = catmullRomThrough(
      [a.position, ...handles.map((record) => record.position), b.position],
      16,
    )
    const points = []
    const previousRef = { current: null }
  ...
  while (usable.length) {
    const drawn = drawThrough(usable)
    if (drawn) return drawn
    let worst = 0
    for (let index = 1; index < usable.length; index += 1) {
      if (usable[index].position[2] < usable[worst].position[2]) worst = index
    }
    usable = usable.filter((_, index) => index !== worst)
  }
  return corridor()
}
...
function skinSegmentPoints(a, b, {
  ...
  if (digitTip) {
    ...
    if (maleHtPinkyPair(fromCode, toCode)) return null
    return [
  ...
  for (let index = 0; index < anchors.length - 1; index += 1) {
    ...
    const skipGeodesic = pairPrefersWrap(
      ...
      to.position,
        ...
        routeNodeCode(anchors[index]),
        routeNodeCode(anchors[index + 1]),
        from.position,
        to.position,
    ...
    const piece = skinSegmentPoints(
      ...
      resolvedNode(anchors[index + 1]),
      {
        ...
        fromCode: routeNodeCode(anchors[index]),
        toCode: routeNodeCode(anchors[index + 1]),
      },
    )
...
function restPathArrays(fromNode, toNode) {
  ...
  const sampled = skinSegmentPoints(a, b, {
    earArc,
    teTemple,
    fromCode,
    toCode,
  })
  const points = (sampled || []).map(toArray)
